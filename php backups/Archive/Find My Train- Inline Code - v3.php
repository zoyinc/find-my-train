...
//
// Function to duplicate the special train info groups
//
window.addEventListener('load', function () {
	divElements = document.getElementsByTagName('div');
	statusDivIndex = -1;
	for (var i=0, im=divElements.length; im>i; i++) {
		if (divElements[i].classList.contains("zoyinc-find_my_train-train_detail_group")){
			statusDivIndex = i;
		};
    };
	if (statusDivIndex == -1){
	...
	defaultInfoGroup = divElements[statusDivIndex];
	defaultInfoGroup.style.display = 'none';	
	currInfoGroup = defaultInfoGroup;
	
	
	currInfoGroup.after(defaultInfoGroup.cloneNode(true));
	currInfoGroup = currInfoGroup.nextSibling;
	currInfoGroup.style.display = '';
	headingElementList = currInfoGroup.getElementsByClassName("zoyinc_info_group_heading");
	if ( headingElementList.length != 1){
		alert("Group info heading not found");
	};
	headingElementList[0].textContent = "New Heading Detail";
	...
	headingElementList[0].textContent = "New Heading Detail 2";
...
//
//window.addEventListener('DOMContentLoaded', updateTrainStatus(), false);
//function updateTrainStatus() {
window.addEventListener('load', function () {
	paragraphElements = document.getElementsByTagName('p');
	statusParagraphIndex = -1;
    for (var i=0, im=paragraphElements.length; im>i; i++) {
		if (paragraphElements[i].classList.contains("zoyinc-find_my_train-train_status")){
			statusParagraphIndex = i;
		};
    };
	if (statusParagraphIndex == -1){
		alert("Current train status message was not found!");
	};
	
<?php	
	// Get the current trains status message
	$query_results = $wpdb->get_results("
		SELECT 
			trip_delay_msg, trip_delay
		FROM 
			fmt_train_details ftd, 
			fmt_trips ft  
		WHERE 
	...
	if ($statusMessage != ""){
		echo "paragraphElements[statusParagraphIndex].innerText = \"" . "train is " . $statusMessage . ".\";";
	} else {
		echo "paragraphElements[statusParagraphIndex].innerText = \"Delay information not available for this train.\";";
	};
?>
